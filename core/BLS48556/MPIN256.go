...
func mpin_hash(sha int, c *FP16, U *ECP) []byte {
	...
	h := core.SPhashit(core.MC_SHA2,sha,t[:])
...
func MPIN_ENCODING(rng *core.RAND, E []byte) int {
	var T [MFS]byte
...
func MPIN_CLIENT_1(sha int, date int, CLIENT_ID []byte, rng *core.RAND, X []byte, pin int, TOKEN []byte, SEC []byte, xID []byte, xCID []byte, PERMIT []byte) int {
	r := NewBIGints(CURVE_Order)
...
*/
func MPIN_GET_G1_MULTIPLE(rng *core.RAND, typ int, X []byte, G []byte, W []byte) int {
	var x *BIG
	r := NewBIGints(CURVE_Order)
	if rng != nil {
		x = Randtrunc(r, 16*AESKEY, rng)
...
func MPIN_CLIENT(sha int, date int, CLIENT_ID []byte, RNG *core.RAND, X []byte, pin int, TOKEN []byte, SEC []byte, xID []byte, xCID []byte, PERMIT []byte, TimeValue int, Y []byte) int {
	rtn := 0
